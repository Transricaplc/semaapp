  ...
  return (
    <div>
      ...
      <div className="mt-6 p-4 rounded-xl bg-primary/10 border border-primary/20 flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        <Button asChild size="sm" className="gap-1.5 bg-primary text-primary-foreground hover:bg-yb-yellow-deep font-bold shrink-0">
...
function RegionAccordion({ region }: { region: LocalRegion }) {
  ...
  return (
    <div className="border border-border rounded-xl overflow-hidden">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center gap-3 px-4 py-3.5 bg-yb-charcoal text-left hover:bg-yb-charcoal-mid transition-colors">
        <div className="w-9 h-9 rounded-lg bg-primary/15 flex items-center justify-center shrink-0">
          <MapPin className="w-4 h-4 text-primary" />
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="font-heading font-bold text-primary text-sm">{region.region_en}</h3>
          <p className="text-[11px] text-yb-charcoal-muted">
    ...
    <div className="border-b border-border/50 last:border-b-0">
      <div className="px-4 py-3 sm:px-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[10px] px-2 py-0.5 rounded-md font-medium bg-primary/10 text-foreground border border-primary/20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 ml-6">
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-card border border-border">
            ...
            {district.ded_phone && (
              <a href={`tel:${district.ded_phone}`} className="ml-auto shrink-0 p-1.5 rounded-md bg-primary/10 text-primary hover:bg-primary/20 transition-colors">
        ...
        {district.wards.length > 0 && (
          <button onClick={() => setWardsOpen(!wardsOpen)} className="flex items-center gap-1.5 mt-2.5 ml-6 text-xs font-medium text-primary hover:underline">
