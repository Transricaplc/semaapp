      ...
      <div className="animate-fade-in">
        <section className="bg-yb-charcoal py-16">
          <div className="container max-w-md text-center">
            ...
            <h1 className="text-2xl font-heading font-bold text-white mb-2">{t("profile.welcome")}</h1>
            <p className="text-yb-charcoal-muted text-sm mb-8">{t("profile.loginPrompt")}</p>
              ...
              {step === "phone" ? (
                <>
                  ...
                  <div className="flex gap-2 mb-4">
                    <div className="bg-yb-charcoal-soft border border-yb-charcoal-soft rounded-xl px-3 flex items-center text-white font-medium text-sm">+255</div>
                    <input type="tel" placeholder="7XX XXX XXX" value={phone} onChange={(e) => setPhone(e.target.value)}
                      className="flex-1 bg-yb-charcoal-soft border border-yb-charcoal-soft rounded-xl px-4 py-3 text-white placeholder:text-yb-charcoal-muted text-base" />
                  </div>
                  <Button onClick={handleSendOTP} disabled={sending} className="w-full bg-primary text-primary-foreground font-bold h-12 text-base hover:bg-yb-yellow-deep">
                    {sending ? "..." : "Send OTP"}
                  </Button>
                </>
              ) : (
                <>
                  <label className="text-sm font-medium text-white/80 mb-2 block">Enter OTP</label>
                  <input type="text" placeholder="XXXXXX" value={otp} onChange={(e) => setOtp(e.target.value)} maxLength={6}
                    className="w-full bg-yb-charcoal-soft border border-yb-charcoal-soft rounded-xl px-4 py-3 text-white placeholder:text-yb-charcoal-muted text-base text-center tracking-[0.3em] mb-4" />
                  <Button onClick={handleVerify} disabled={sending} className="w-full bg-primary text-primary-foreground font-bold h-12 text-base hover:bg-yb-yellow-deep">
                    {sending ? "..." : "Verify"}
                  </Button>
                  <button onClick={() => setStep("phone")} className="w-full text-xs text-yb-charcoal-muted mt-3 hover:underline">← Back</button>
                </>
              )}
            </div>
            ...
            <div className="mt-6 flex items-center gap-3 justify-center">
              ...
              <span className="text-xs text-yb-charcoal-muted">or</span>
              <div className="h-px flex-1 bg-white/10" />
            </div>

            <Button variant="outline" onClick={handleAnonymous} className="mt-4 w-full border-yb-charcoal-soft text-white bg-transparent hover:bg-white/5 gap-2">
        ...
        <div className="container max-w-3xl">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-yb-charcoal-muted">{user.phone || user.email || "Anonymous"}</p>
            </div>
            <Button variant="outline" size="sm" onClick={signOut} className="border-yb-charcoal-soft text-white gap-1.5 hover:bg-white/5">
      ...
      <div className="container max-w-3xl py-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          {impactStats.map((stat) => (
            <div key={stat.label} className="yb-card p-4 text-center">
        ...
        <div className="space-y-3">
          {mockReports.slice(0, 3).map((report) => (
            <div key={report.id} className="yb-card p-4 flex items-center gap-3">
